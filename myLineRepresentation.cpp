...
void myLineRepresentation::SetDirectionalLine(bool val) {
  ...
  this->HandleMapper[1]->SetInputConnection(
      this->HandleGeometry[1]->GetOutputPort());
}
...
void myLineRepresentation::StartWidgetInteraction(double e[2]) {
  ...
  if (this->InteractionState == myLineRepresentation::Scaling) {
    ...
    this->Length = sqrt((dp1[0] - dp2[0]) * (dp1[0] - dp2[0]) +
...
void myLineRepresentation::WidgetInteraction(double e[2]) {
  if (this->InteractionState == myLineRepresentation::OnLine) {
    ...
    double delta = sqrt((this->StartEventPosition[0] - e[0]) *
                            (this->StartEventPosition[0] - e[0]) +
                        (this->StartEventPosition[1] - e[1]) *
                            (this->StartEventPosition[1] - e[1]));
...
void myLineRepresentation::SizeHandles() {
  ...
  radius = this->vtkWidgetRepresentation::SizeHandlesInPixels(
      1.35, this->LineSource->GetPoint2());
  if (this->DirectionalLine) {
    static_cast<vtkConeSource *>(this->HandleGeometry[1])->SetRadius(radius);
    static_cast<vtkConeSource *>(this->HandleGeometry[1])
        ->SetHeight(2.8 * radius);
  } else {
...
void myLineRepresentation::BuildRepresentation() {
  ...
  if (this->GetMTime() > this->BuildTime ||
    ...
    // Make sure that tolerance is consistent between handles and this
    // representation
    ...
    double x[3] = {(x1[0] + x2[0]) / 2.0, (x1[1] + x2[1]) / 2.0,
                   (x1[2] + x2[2]) / 2.0};
    char string[512];
    snprintf(string, sizeof(string), this->DistanceAnnotationFormat,
             this->Distance);
    ...
    if (!this->AnnotationTextScaleInitialized) {
      ...
      this->TextActor->SetScale(this->Distance / 10.0, this->Distance / 10.0,
                                this->Distance / 10.0);
    }
...
void myLineRepresentation::HighlightPoint(int ptId, int highlight) {
  if (ptId == 0) {
    if (highlight) {
      ...
      this->Point1Representation->SetSelectedProperty(
    ...
    if (highlight) {
      ...
      this->Point2Representation->SetSelectedProperty(
  ...
  {
    if (highlight) {
      this->LineHandleRepresentation->SetSelectedProperty(
          this->SelectedEndPointProperty);
    } else {
      this->LineHandleRepresentation->SetProperty(this->EndPointProperty);
    }
...
void myLineRepresentation::PrintSelf(ostream &os, vtkIndent indent) {
  ...
  if (this->SelectedEndPointProperty) {
    os << indent
       << "Selected End Point Property: " << this->SelectedEndPointProperty
       << "\n";
  } else {
  ...
  if (this->SelectedEndPoint2Property) {
    os << indent
       << "Selected End Point Property: " << this->SelectedEndPoint2Property
       << "\n";
  } else {
    os << indent << "Selected End Point Property: (none)\n";
  }
  ...
  os << indent
     << "Constrain To Bounds: " << (this->ClampToBounds ? "On\n" : "Off\n");
